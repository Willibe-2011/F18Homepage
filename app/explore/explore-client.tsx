...
export function ExploreClient({ profiles }: ExploreClientProps) {
  ...
  const filteredProfiles = useMemo(() => {
    let filtered = profiles.filter(p => p.name !== "Kavin Ramadoss" && p.name !== "Bhuvika Tripuraneni" && p.name !== "Omuwa Izah" && p.name !== "Mohammad Parsa Parhizkar" && p.name !== "Samuel Montanini" && p.name !== "Siyaa Poddar")
